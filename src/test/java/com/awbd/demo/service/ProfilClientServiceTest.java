...
class ProfilClientServiceTest {
    ...
    void create_shouldThrowException_whenClientAlreadyHasProfil() {
        ...
        verify(profilRepo, never()).save(any());
    }
    ...
    void getAll_shouldReturnAllProfiles() {
        ...
        when(profilRepo.findAll()).thenReturn(Arrays.asList(p1, p2));

        List<ProfilClient> result = service.getAll();

        assertEquals(2, result.size());
        assertEquals("Antigripal", result.get(0).getVaccinari());
        assertEquals("COVID", result.get(1).getVaccinari());

        verify(profilRepo).findAll();
    }
    ...
    void delete_shouldThrowException_whenNotFound() {
        ...
        verify(profilRepo).findById(99L);
        verify(profilRepo, never()).delete(any());
    }
}