...
class CardFidelitateServiceTest {
    ...
    void getAll_shouldReturnAllCards() {
        ...
        when(cardRepo.findAll()).thenReturn(Arrays.asList(c1, c2));

        List<CardFidelitate> result = service.getAll();

        assertEquals(2, result.size());
        assertEquals("silver", result.get(0).getNivel());
        assertEquals("gold", result.get(1).getNivel());
