...
class ClientProfilClientIntegrationTest {
    ...
    void createClient_thenCreateProfil_thenFetchProfilById() {
        ...
        Page<ProfilClient> allProfiles = profilClientService.getAll(PageRequest.of(0, 10));

        assertEquals(1, allProfiles.getContent().size());
        assertEquals("Antigripal", allProfiles.getContent().get(0).getVaccinari());
    }
}