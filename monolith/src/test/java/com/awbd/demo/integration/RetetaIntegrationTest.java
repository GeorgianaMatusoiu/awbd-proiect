...
class RetetaIntegrationTest {
    ...
    void createClientAndFarmacist_thenCreateReteta_thenFetchRetetaById() {
        ...
        Page<Reteta> allRetete = retetaService.getAll(PageRequest.of(0, 10));

        assertEquals(1, allRetete.getContent().size());
        assertEquals(LocalDate.of(2026, 3, 8), allRetete.getContent().get(0).getDataTiparire());
    }
}