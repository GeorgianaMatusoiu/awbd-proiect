...
class ProspectServiceTest {
    ...
    void getAll_shouldReturnAllProspects() {
        ...
        Page<Prospect> page = new PageImpl<>(Arrays.asList(p1, p2), pageable, 2);

        when(repo.findAll(pageable)).thenReturn(page);

        Page<Prospect> result = service.getAll(pageable);

        assertEquals(2, result.getContent().size());
        assertEquals("Durere", result.getContent().get(0).getAfectiuni());
        assertEquals("Febra", result.getContent().get(1).getAfectiuni());
