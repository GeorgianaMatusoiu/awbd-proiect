...
class RetetaServiceTest {
    ...
    void getAll_shouldReturnAllRetete() {
        ...
        Page<Reteta> page = new PageImpl<>(Arrays.asList(r1, r2), pageable, 2);

        when(retetaRepo.findAll(pageable)).thenReturn(page);

        Page<Reteta> result = service.getAll(pageable);

        assertEquals(2, result.getContent().size());
        assertEquals(LocalDate.of(2026, 3, 8), result.getContent().get(0).getDataTiparire());
        assertEquals(LocalDate.of(2026, 3, 9), result.getContent().get(1).getDataTiparire());

        verify(retetaRepo).findAll(pageable);
    }
